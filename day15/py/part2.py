...
for i,line in enumerate(lines): #I later realized that the actual ingredients were not needed
    ...
    subdict[line[9]]=int(line[10]) #include calories for part 2
...
calories=[mydict[x]["calories"] for x in mydict.keys()] #In Part 2 we need this list
print(mydict2)
...
for i in itertools.filterfalse(lambda x : sum(x) != 100,mypermutations): #iterate over each permutation but filter to only those that sum to 100
    if(dotproduct(i, calories) ==500):#In Part 2 only keep the ones where dot product is 500
        thismax=1
